...
class Companies(scrapy.Spider):
    ...
    def __init__(self, category, main_rubric, rubric_id):
        ...
        self.start_urls = ['https://www.tripadvisor.ru/RestaurantSearch?Action=PAGE&ajax=1&availSearchEnabled=false&sortOrder=popularity&geo=2324040&itags={}'
                            .format(self.rubric_id)]
        self.result = list()
        self.count = 0
...
class Companies_Phone(scrapy.Spider):
    ...
    def parse(self, response, **kwargs):
        ...
        try:
            self.address_name = response.css('.AYHFM::text').extract_first().split(',')

            if len(self.address_name) > 1:
                street = self.address_name[0].strip().lower()
                house = self.address_name[1].split()[0].lower()
                self.address_name = ' '.join([street, house])

            for j in self.clear_addr:
                if j in self.address_name:
                    self.address_name = re.sub(j, '', self.address_name).strip()
            self.address_name = re.sub(' +', ' ', self.address_name)
    ...
    def close(self, reason):
        connection = sqlite3.connect('db/{}.db'.format(self.category))
        cursor = connection.cursor()
        cursor.execute("UPDATE trip SET phone=?, address=? WHERE url=?", (self.phone, self.address_name, self.company_link))
        connection.commit()
        connection.close()
